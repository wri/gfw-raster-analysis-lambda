...
def get_shapely_geom():
    ...
    if len(geojson['features']) > 1:
        raise Exception('Currently accepting only 1 feature at a time')
    ...
    if area_ha > 10000000:
        raise Exception('Geometry is too large for custom stats/download. ' \
...
def parse_download_format(fmt):
    if fmt not in ['csv', 'json']:
        raise Exception("format must be one of ['csv', 'json']")
...
def check_dates(period):
    try:
        ...
        raise Exception('period must be formatted as YYYY-mm-dd,YYYY-mm-dd')
    ...
    if start_date < earliest_date:
        raise Exception('Start date must be later than Jan 1, 2015')
